...
int main(int argc, string argv[])
{
    ...
    {
        printf("%d's pair winner: %s\n", i, candidates[pairs[i].winner]);
        printf("%d's pair loser: %s\n\n", i, candidates[pairs[i].loser]);
    }
    printf("Sorted pairs:\n");
    sort_pairs();
    for (int i = 0; i < pair_count; i++)
    {
        printf("Pair %d: %s (%d) - %s (%d)\n", i, candidates[pairs[i].winner], preferences[pairs[i].winner][pairs[i].loser], candidates[pairs[i].loser], preferences[pairs[i].loser][pairs[i].winner]);
    }
    lock_pairs();
    printf("\nLocked pairs:\n");
    for (int i = 0; i < pair_count; i++)
